...
### The coroutine is the scheduler's, not the core's

The core defines **no** coroutine class or accessor: the coroutine object, how a
running coroutine finds itself, and how it is woken are all the scheduler's own
API. Here the scheduler keeps a `Fiber -> coroutine` map and exposes
`currentCoroutine()`; waking is just its own `resume()` hook (re-queue). A
different scheduler could do it entirely differently.
...
itself via the scheduler (currentCoroutine) and suspends; another resolves the
future and wakes it through the scheduler (its resume hook).
